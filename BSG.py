"""Generating input-size vs execution Graph for Binary Search Algorithm. """
...
for size in range(10000, 110000, 10000):
    ...
    x2axis.append(size)
    start1 = time.time()
    BinarySearch(list(range(size)), (size-1)//2) # Middle element in the list.
    end1 = time.time()
    time_taken1 = "%.2f" % (end1-start1)
    y1axis.append(time_taken1)
    start2 = time.time()
    BinarySearch(list(range(size)), -100) # -100 is not in the list.
    end2 = time.time()
